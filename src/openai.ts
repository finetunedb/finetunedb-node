...
import {
  FinetuneDbCompletionArgs,
  type FinetuneDbClientOptions,
  FinetuneDbCompletionMeta,
} from "./shared";
...
export type ClientOptions = openai.ClientOptions & {
  finetunedb?: FinetuneDbClientOptions;
};
...
export default class OpenAI extends openai.OpenAI {
  ...
  constructor({ finetunedb, ...options }: ClientOptions = {}) {
    ...
    const finetuneDbApiKey =
      finetunedb?.apiKey ?? readEnv("FINETUNEDB_API_KEY");
    const finetuneDbBaseUrl =
      finetunedb?.baseUrl ?? "https://app.finetunedb.com/api/v1";
    const finetuneDbProjectId =
      finetunedb?.projectId ?? readEnv("FINETUNEDB_PROJECT_ID");
...
class WrappedCompletions extends openai.OpenAI.Chat.Completions {
  ...
  async create(
    {
      finetunedb: rawFinetunedb,
      ...body
    }: ChatCompletionCreateParams & FinetuneDbCompletionArgs,
    options?: Core.RequestOptions,
  ): Promise<
    Core.APIPromise<
      | (ChatCompletion & { finetunedb: FinetuneDbCompletionMeta })
      | WrappedStream
    >
  > {
    const finetunedb = {
...
class WrappedEmbeddings extends openai.OpenAI.Embeddings {
  ...
  create(
    ...
    CreateEmbeddingResponse & { finetunedb: FinetuneDbCompletionMeta }
  >;
  async create(
    {
      finetunedb: rawFinetunedb,
      ...body
    }: EmbeddingCreateParams & FinetuneDbCompletionArgs,
    options?: Core.RequestOptions,
  ): Promise<
    Core.APIPromise<
      CreateEmbeddingResponse & { finetunedb: FinetuneDbCompletionMeta }
